...
STUDY_DIR="${RUN_DIR}/optimization_ideas_als_chm_flexible_lmf_v3"
INCUMBENT="${RUN_DIR}/optimization_ideas_als_source_balanced_loso_v2/best_parameters.json"
FOR_INSTANCE_DIR="${RUN_DIR}/optimization_for_instance_dev_weighted_pq_v2_refined/dev_input"
TRANSFER_DIR="${RUN_DIR}/evaluation_for_instance_ideas_als_chm_flexible_lmf_v3_selected"
BEST_NEW="${STUDY_DIR}/tune/trial_0044_02b6711f1bf0/result.json"
BEST_NEW_TRANSFER_DIR="${RUN_DIR}/evaluation_for_instance_ideas_als_chm_flexible_lmf_v3_best_new"
...
"${PYTHON}" "${SCRIPT_DIR}/optimize_ideas_als_chm_lmf_v3.py" \
...
# Reproduce the explicitly labelled best-new post-selection ablation.
"${PYTHON}" "${SCRIPT_DIR}/evaluate_best_parameters.py" \
  --best-parameters "${BEST_NEW}" \
  --input-dir "${FOR_INSTANCE_DIR}" \
  --gt-dir "${FOR_INSTANCE_DIR}" \
  --output-dir "${BEST_NEW_TRANSFER_DIR}"
